...
export default function DaftarLemburSaya({ daftarAwal }: { daftarAwal: LemburSaya[] }) {
  ...
  return (
    <div className="space-y-3">
      <div className="mb-1 flex items-center justify-between rounded-[20px] border border-cream-200 bg-white/80 px-3 py-2 shadow-sm">
        <div>
          <p className="text-[11px] font-black tracking-[0.25em] text-maroon-800">RIWAYAT LEMBUR</p>
          <p className="text-xs text-muted">{daftar.length} pengajuan tersimpan</p>
        </div>
        <div className="rounded-2xl bg-cream-100 px-3 py-1.5 text-[11px] font-semibold text-maroon-700">
          Terbaru dulu
        </div>
      </div>
      {daftar.map((l) => (
        <div key={l.id} className="rounded-[22px] border border-cream-200 bg-white/95 p-4 shadow-sm transition hover:border-maroon-200 hover:shadow-md">
